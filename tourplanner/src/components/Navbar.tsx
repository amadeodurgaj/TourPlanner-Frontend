...
export const Navbar = ({
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/60 bg-primary/70 backdrop-blur-xl">
      <nav className="mx-auto grid h-navbar max-w-6xl grid-cols-[auto_1fr_auto] items-center px-6">
        <NavLink
          to="/"
          className="group relative font-serif text-3xl font-bold tracking-tighter text-secondary transition-all duration-300 hover:text-accent cursor-pointer"
        >
          ...
          {navLinks.map((link) => (
            <li key={link.name}>
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "relative font-serif text-xl font-light tracking-wide transition-all duration-200 cursor-pointer",
                    isActive
        ...
        <div className="hidden md:flex items-center gap-6 ml-auto">
          <button
            className="flex items-center justify-center w-9 h-9 rounded-full text-secondary hover:bg-accent/10 hover:text-accent transition-all duration-300 cursor-pointer"
            ...
            {navLinks.map((link) => (
              <li key={link.name}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn(
                      "block font-serif text-base tracking-wide py-1 transition-colors duration-200 cursor-pointer",
                      isActive
                        ? "text-secondary"
                        : "text-muted hover:text-secondary",
                    )
